...
class SAM2VideoPredictor(SAM2Base):
    ...
    @torch.inference_mode()
    def bidirectional_propagation(self, inference_state, start_frame_idx):
        """
        --- INJECT (Step 11) ---
        Custom wrapper for Bidirectional Propagation in 3D Medical Volumes.
        Given a starting slice index (e.g., the slice with the RL agent's Bbox),
        this function propagates the mask sequentially to both ends of the Z-axis.
        """
        num_frames = inference_state["num_frames"]
        all_video_res_masks = [None] * num_frames
        
        # We assume the initial conditioning frame has already received the prompt 
        # (e.g., via `add_new_points_or_box`) and is stored in `inference_state`.
        
        # 1. Propagate Forward (Positive Z direction)
        if start_frame_idx < num_frames - 1:
            print(f"Starting Forward Propagation from slice {start_frame_idx} to {num_frames-1}")
            for frame_idx, obj_ids, video_res_masks in self.propagate_in_video(
                inference_state, 
                start_frame_idx=start_frame_idx, 
                reverse=False
            ):
                # The yield includes the start_frame_idx as well
                all_video_res_masks[frame_idx] = video_res_masks

        # 2. Propagate Backward (Negative Z direction)
        if start_frame_idx > 0:
            print(f"Starting Backward Propagation from slice {start_frame_idx} to 0")
            for frame_idx, obj_ids, video_res_masks in self.propagate_in_video(
                inference_state, 
                start_frame_idx=start_frame_idx, 
                reverse=True
        ...
        # If there is only one frame, the loop above handles it in the Forward pass yield
        
        return all_video_res_masks
