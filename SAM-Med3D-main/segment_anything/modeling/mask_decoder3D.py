        ...
        *,
        ...
        Arguments:
          ...
          num_multimask_outputs (int): the number of masks to predict
            when disambiguating masks (only used in single-class mode)
          num_classes (int): number of classes for multi-class segmentation.
            If > 2, uses multi-class mode (one token per class).
        ...
        # For multi-class: num_mask_tokens = num_classes (one per class)
        # For single-class: num_mask_tokens = num_multimask_outputs + 1
        if num_classes > 2:
            # Multi-class mode
            self.num_mask_tokens = num_classes
        else:
            # Single-class (binary) mode - original behavior
