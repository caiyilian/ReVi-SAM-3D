...
def get_next_click3D_torch_multiclass(prev_masks, gt_semantic_seg, num_classes):
    """Generate prompt clicks for multi-class semantic segmentation.
    
    Args:
        prev_masks (torch.Tensor): Previous predictions, shape (B, num_classes, D, H, W)
        gt_semantic_seg (torch.Tensor): Ground truth semantic labels, shape (B, 1, D, H, W) 
                                       with values 0, 1, ..., num_classes-1
        num_classes (int): Number of classes (including background)
    
    Returns:
        batch_points (list of list): points[i] contains points for class i
        batch_labels (list of list): labels[i] contains labels (0/1) for class i
    """
    mask_threshold = 0.5
    
    # Lists to store points and labels for each class
    batch_points_all_classes = []
    batch_labels_all_classes = []
    
    # Process each class separately (skip class 0 which is background)
    for class_id in range(1, num_classes):
        batch_points = []
        batch_labels = []
        
        # Get predictions and GT for this class
        prev_masks_cls = prev_masks[:, class_id, ...]  # (B, D, H, W)
        gt_masks_cls = (gt_semantic_seg == class_id)  # (B, 1, D, H, W)
        
        # Compute FN and FP
        pred_masks = prev_masks_cls > mask_threshold
        true_masks = gt_masks_cls > 0
        
        fn_masks = torch.logical_and(true_masks, torch.logical_not(pred_masks))
        fp_masks = torch.logical_and(torch.logical_not(true_masks), pred_masks)
        to_point_mask = torch.logical_or(fn_masks, fp_masks)
        
        # Generate points for each sample in batch
        for i in range(gt_semantic_seg.shape[0]):
            points = torch.argwhere(to_point_mask[i, 0, ...])  # (num_points, 3)
            
            if len(points) > 0:
                # Randomly select a point
                point = points[np.random.randint(len(points))]
                
                # Determine if positive (FN) or negative (FP) click
                if fn_masks[i, 0, point[0], point[1], point[2]]:
                    is_positive = True
                else:
                    is_positive = False
            else:
                # If no FN/FP points, random location
                point = torch.Tensor([np.random.randint(sz)
                                    for sz in to_point_mask[i, 0, ...].size()]).to(torch.int64)
                is_positive = False
            
            # Format: (1, 1, 3) for coordinates
            bp = point[0:].clone().detach().reshape(1, 1, 3)
            bl = torch.tensor([int(is_positive)]).reshape(1, 1)
            
            batch_points.append(bp)
            batch_labels.append(bl)
        
        batch_points_all_classes.append(batch_points)
        batch_labels_all_classes.append(batch_labels)
    
    return batch_points_all_classes, batch_labels_all_classes
